...
const CampaignDetail = () => {
  ...
  // ✅ FIXED: correct mapping (toEmail instead of email)
  const logMap = Object.fromEntries(logs.map((l) => [l.toEmail, l]));

  // ✅ FIXED: proper status logic
  const getStatus = (email) => {
    const log = logMap[email];

    if (!log) return "NOT_SENT";
    if (log.status === "FAILED") return "FAILED";
    if (log.status === "SENT" && log.opened) return "OPENED";
    if (log.status === "SENT" && !log.opened) return "NOT_OPENED";
    ...
    <div className="relative z-10 max-w-7xl mx-auto">
      ...
      <div className="group relative overflow-hidden bg-white/60 backdrop-blur-3xl border border-white/60 shadow rounded-3xl p-6 md:p-8 mb-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-6">
          <p>
            <b>Start:</b> {campaign.startDate}
          </p>
          <p>
            <b>End:</b> {campaign.endDate}
          </p>
          <p>
            <b>Region:</b> {campaign.region}
          </p>
          <p>
            <b>Language:</b> {campaign.language}
          </p>
          <p>
